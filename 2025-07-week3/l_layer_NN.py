...
class l_layer_NN():
  ...
  def __init__(self,activation, layer_units = [7,4,3,1], l_rate=0.001, n_iter=1000):
  ...
  def params_ini(self,X):
    ...
    for i in range(len(self.layer_units)):
      if i == 0:
        ...
        self.params[f"b{i+1}"] = np.random.randn(self.layer_units[0],1)

      else:
        self.params[f"W{i+1}"] = np.random.randn(self.layer_units[i],self.params[f"W{i}"].shape[0])  * np.sqrt(2/self.params[f"W{i}"].shape[0])
        self.params[f"b{i+1}"] = np.random.randn(self.layer_units[i],1)
  ...
  def loss_compute(self,y_pred,y):
    ...
    if self.activation:
      ...
      cost = np.sum(- (y*np.log(y_pred)) - (1-y*np.log(1-y_pred))) #for binary output
    else:
  ...
  def back_prop(self, y): #calculates gradient for a single layer
    ...
    for l in range(int(len(self.forward_cache)/2),0,-1):
      dA = self.forward_cache[f"A{l}"] - y
      if self.activation == "sigmoid":
        dZ = dA * self.forward_cache[f"A{l}"] * (1-self.forward_cache[f"A{l}"])
      else:
        dZ = dA * (self.forward_cache[f"A{l}"]>0) # if it's sigmoid
