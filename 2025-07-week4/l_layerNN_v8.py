...
# Finally something that works and gives good results on stuff

#reshapes data  to suit Neural Net better
# def reshape(X,Y):
...
#Makes a one hot vector out of y, best for working with multiclass
# def onehot_y(y,n_classes):
#   m = y.shape[1] # Get the number of samples from the first dimension
#   y = y.astype(int)
#   y_flatten = y.flatten()
#   y_onehot = np.zeros((n_classes,m))
#   y_onehot[y_flatten,np.arange(m)] = 1 # Use np.arange(m) for column indices
...
class l_layer_NN():
  ...
  def __init__(self,final_activ="sigmoid", layer_units = [7,4,3,1], l_rate=0.01, epochs=50, mini_batch = True, batch_size=128, use_adam = True):
  ...
  def back_prop(self, y): #calculates gradient for a single layer
    ...
    # print(np.min(self.grads["dW1"]),np.max(self.grads["dW1"]),self.grads["dW1"])
  ...
  def gradient_descent(self):
    ...
    for l in range(1,int(len(self.params)/2)+1):
      ...
      if self.use_adam:
        self.params[f"W{l}"] -= (self.l_rate * adam_res[f"WFM{l}"]) / (np.sqrt(adam_res[f"WSM{l}"]) + self.epsilon)
        self.params[f"b{l}"] -= (self.l_rate * adam_res[f"BFM{l}"]) / (np.sqrt(adam_res[f"BSM{l}"]) + self.epsilon)
  ...
  def adam(self):
    ...
    for i in range(n_layers):
      ...
      # print(m_m_hat/m_m_hat, v_m_hat/v_m_hat, m_b_hat/m_b_hat, v_b_hat/v_b_hat)
      ...
      # print(m_m_hat, v_m_hat, m_b_hat, v_b_hat)
      # print("dW", dW, "db", db)
  ...
  def train(self,X,Y):
    ...
    self.params_ini(X)
    a = time.time()
    for epoch in range(self.epochs): #just for now
      ...
      if self.mini_batch:
        ...
        b = time.time()
        print(f"Cost for epoch {epoch} is", cost, f"Time it took for the last 100 iterations is {b-a}") #epoch!=iteration (most cases)
        a = time.time()
