...
class l_layer_NN():
  ...
  def params_ini(self,X):
    ...
    #HELPS IN ADAM LATER
      if self.use_adam:
          self.m_m[i] = np.zeros_like(self.params[f"W{i+1}"])
          self.v_m[i] = np.zeros_like(self.params[f"W{i+1}"])
          self.m_b[i] = np.zeros_like(self.params[f"b{i+1}"])
          self.v_b[i] = np.zeros_like(self.params[f"b{i+1}"])
  ...
  def adam(self):
    ...
    for i in range(n_layers):
      ...
      self.v_m[i+1] = self.beta2 * (self.v_m[i+1]) + (1-self.beta2) * (db)

      self.m_b[i+1]  = self.beta1 * (self.m_b[i+1]) + (1-self.beta1) * (dW**2)
